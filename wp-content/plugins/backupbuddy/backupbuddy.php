	...
	class pluginbuddy_backupbuddy {
		...
		function send_remote_destination( $destination_id, $file ) {
			...
			if ( $destination['type'] == 's3' ) {
				...
				$response = $this->remote_send_rackspace( $destination['username'], $destination['api_key'], $destination['container'], $file, $destination['archive_limit'], $destination['server'] );
						...
						foreach( $results as $backup ) {
							...
							if ( $i > $limit ) {
								if ( !ftp_delete( $conn_id, $backup ) ) {
									$this->log( 'Unable to delete excess FTP file `' . $backup . '` in path `' . $path . '`.' );
									$delete_fail_count++;
								}
							}
						}
		...
		function test_ftp( $server, $username, $password, $path, $type = 'ftp' ) {
			...
			if ( $type == 'ftp' ) {
				$conn_id = @ftp_connect( $server, $port );
				if ( $conn_id === false ) {
					return 'Unable to connect to FTP address `' . $server . '` on port `' . $port . '`.';
				}
			} else {
				if ( function_exists( 'ftp_ssl_connect' ) ) {
					$conn_id = @ftp_ssl_connect( $server, $port );
